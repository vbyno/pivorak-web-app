Rails.application.routes.draw do
  ...
  devise_for :users, class_name: 'Authentication::User', controllers: {
    omniauth_callbacks: 'authentication/omniauth/callbacks',
  ...
  namespace :admin do
    ...
    resources :members, only: :index, controller: 'gathered_members'
    resources :members, except: %i[show destroy index] do
      resource :squash, only: %i[show create], controller: 'members/squash'
      post     :sign_in_as,                    to: 'members/sign_in_as#create'
      resource :admin_access, only: %i[update destroy], controller: 'members/admin_access'
    end
